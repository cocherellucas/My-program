...
import { buildActivationResult } from '@/lib/program-activation';
...
export default function Program() {
  ...
  useEffect(() => {
    // Attendre que les objectifs soient CHARGÉS depuis la base avant de générer,
    // sinon findMatchingProgram ne voit aucun objectif → "aucun programme ne
    // correspond" (course au montage).
    if (!configOpenedRef.current && user && objectives.length > 0 && !activeProgram && !generating && searchParams.get('configureProgram') === 'true') {
  ...
  useEffect(() => {
    if (user && objectives.length > 0 && !activeProgram && !generating && !autoGenerated.current && searchParams.get('autoGenerate') === 'true') {
      ...
      try {
        // Programme en boucle sélectionné selon le profil — aucun réglage à calculer.
        generateProgram({ phase: 'MEV' });
      } catch (err) {
  ...
  const generateProgram = async ({ structure, weeks, phase }) => {
    ...
    // ── Activation d'un programme pré-généré (plus d'appel IA) ──────────────
  ...
  return (
    ...
    <div className="space-y-6">
                 ...
                 {weekSessions.map((session, i) => {
                   ...
                   return (
                     ...
                     >
                       ...
                       {/* blur() : sinon le lien garde le focus et, au retour sur Programme (overflow
                           hidden→auto), le navigateur re-scrolle vers la séance focalisée → scroll bloqué. */}
                       <Link to={`/session?id=${session.id}${session.status === 'completed' ? '&edit=true' : ''}`} onClick={(e) => e.currentTarget.blur()}>{card}</Link>
                     </motion.div>
                   );
                 })}
              </div>
      ...
      <AlertDialog open={showRegenGate} onOpenChange={setShowRegenGate}>
        <AlertDialogContent className="bg-violet-800 border-violet-700 text-white">
          ...
          <div className="flex flex-col gap-2 py-2">
            <button
              onClick={async () => { try { await saveProgram(); setShowRegenGate(false); generateProgram({}); } catch {} }}
            ...
            <button
              onClick={async () => { setShowRegenGate(false); await deleteProgram(); generateProgram({}); }}
      ...
      {onProgramPage && programFinished && !sessionsLoading && !showRegenGate && !generating && !deleting && createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-6 bg-black/50 backdrop-blur-md">
          <div className="w-full max-w-sm rounded-2xl p-5 space-y-4" style={{ background: 'linear-gradient(160deg, #2e1065, #1e0050)', border: '1px solid rgba(255,255,255,0.15)' }}>
            ...
            <div className="space-y-2">
              ...
              {canGenerate ? (
                <button onClick={async () => { await deleteProgram(); generateProgram({}); }} disabled={generating}
