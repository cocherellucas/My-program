    ...
    <div className="space-y-3 mb-6">
      <div className="flex items-center gap-2 text-sm">
        ...
        <span className="font-medium text-white">
          {session.planned_date && format(new Date(session.planned_date), 'EEEE d MMMM', lang === 'fr' ? { locale: fr } : undefined)}
        </span>
        ...
        <div className="relative">
          ...
          <Link to={`/session?id=${todaySession.id}`}>
            <Button className="w-full bg-white text-violet-700 hover:bg-white/90 font-semibold">
              ...
              {isInProgress ? t('resume_session') : t('start_session')}
            </Button>
          </Link>
        </div>
      </Card>
    ...
    return (
      <Card className="p-6 bg-white/15 backdrop-blur-sm border-white/20">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="font-heading font-bold text-base text-white">{t('rest_today')}</h3>
            <p className="text-xs text-white/60">{t('rest_sub')}</p>
          </div>
        </div>
        {nextSession && (
          <div className="pt-4 border-t border-white/20">
            <p className="text-xs text-white/50 mb-2 uppercase tracking-wide font-semibold">{t('next_session')}</p>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-semibold text-white">{(nextSession.day_label || nextSession.type || '').replace(/\s*§\d+/g, '')}</p>
                <p className="text-xs text-white/60">
                  {nextSession.planned_date && format(new Date(nextSession.planned_date), 'EEEE d MMMM', lang === 'fr' ? { locale: fr } : undefined)}
  ...
  return (
    <Card className="p-6 bg-white/15 backdrop-blur-sm border-white/20">
      <div className="flex flex-col items-center text-center gap-4">
        <div>
          <h3 className="font-heading font-bold text-base text-white mb-1">{t('no_program_title')}</h3>
          <p className="text-xs text-white/60">{t('no_program_sub')}</p>
        </div>
